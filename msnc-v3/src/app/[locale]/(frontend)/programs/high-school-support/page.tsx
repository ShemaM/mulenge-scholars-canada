...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'HighSchoolSupportPage.metadata' })
  
  return { 
    title: `${t('title')} | ${SITE_NAME}`, 
    description: t('description'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/programs/high-school-support`,
    },
    openGraph: {
      title: `${t('title')} | ${SITE_NAME}`,
      description: t('description'),
      url: `${SITE_URL}/${locale}/programs/high-school-support`,
      siteName: SITE_NAME,
      locale: locale,
      type: 'website',
    },
...
const modules = [
  { icon: UserCheck, bgClass: 'bg-secondary/10', iconClass: 'text-secondary', key: 'tutoring', wide: true },
  { icon: MapPin, bgClass: 'bg-paper-50 border border-border', iconClass: 'text-primary', key: 'courses', wide: false },
  { icon: Compass, bgClass: 'bg-paper-50 border border-border', iconClass: 'text-primary', key: 'planning', wide: false },
  { icon: BookOpen, bgClass: 'bg-secondary/10', iconClass: 'text-secondary', key: 'applications', wide: false },
  { icon: Trophy, bgClass: 'bg-paper-50 border border-border', iconClass: 'text-primary', key: 'leadership', wide: false },
] as const

export default async function HighSchoolSupportPage({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'HighSchoolSupportPage' })
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-paper-50 pb-20 pt-32 md:pt-40 border-b border-border">
        ...
        <div className="container-editorial relative z-10">
          ...
          <div className="max-w-4xl space-y-8">
            ...
            {/* Stat Strip: Strategic Location */}
            <div className="flex flex-wrap gap-10 border-t border-border pt-10 mt-12">
              {[
                { icon: TrendingUp, value: '88%', label: t('statsPlacement') },
                { icon: Users, value: '150+', label: t('statsMentored') },
                { icon: Award, value: '5', label: t('statsModules') },
      ...
      {/* ── Chapter II: Methodology (Asymmetric Editorial Grid) ────────────── */}
      <section className="section border-y border-border bg-white py-24">
        <div className="container-editorial">
          ...
          {/* Navigational Jump Links */}
          <nav aria-label="Program modules navigation" className="mb-12 flex flex-wrap gap-4">
            {modules.map((mod) => (
              <a
                key={`nav-${mod.key}`}
                href={`#${mod.key}`}
                className="inline-flex items-center gap-2 rounded-full border border-border bg-paper-50 px-6 py-2 text-xs font-bold uppercase tracking-widest text-primary transition-colors hover:border-secondary hover:text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary focus-visible:ring-offset-2"
              >
            ...
            {modules.map((mod, i) => {
              ...
              return (
                <article
                  ...
                  id={mod.key}
                  className={[
                    'group scroll-mt-32 flex flex-col rounded-[2rem] border border-border bg-paper-50 p-10 transition-all duration-500 hover:border-secondary hover:bg-white hover:shadow-xl focus-within:ring-2 focus-within:ring-secondary focus-within:ring-offset-2',
            ...
            <div className="grid items-center gap-12 lg:grid-cols-2">
              ...
              <div className="flex flex-col gap-4 lg:items-end">
                <Link 
                  ...
                  className="btn btn-primary h-16 px-10 w-full lg:w-auto text-lg group"
                >
                  {t('cta.primary')}
                  <ArrowRight className="ml-3 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link href="/contact" className="btn btn-outline h-16 px-10 w-full lg:w-auto text-lg">
                  {t('cta.secondary')}
                </Link>
              </div>
            </div>
