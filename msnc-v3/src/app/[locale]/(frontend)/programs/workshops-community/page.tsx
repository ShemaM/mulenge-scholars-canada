...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'WorkshopsPage.metadata' })
  
  return { 
    title: `${t('title')} | ${SITE_NAME}`, 
    description: t('description'),
    alternates: {
      canonical: `${SITE_URL}/${locale}/programs/workshops-community`,
    },
    openGraph: {
      title: `${t('title')} | ${SITE_NAME}`,
      description: t('description'),
      url: `${SITE_URL}/${locale}/programs/workshops-community`,
      siteName: SITE_NAME,
      locale: locale,
      type: 'website',
    },
...
const modules = [
  { icon: Lightbulb, bgClass: 'bg-secondary/10', iconClass: 'text-secondary', key: 'academic' },
  { icon: Briefcase, bgClass: 'bg-paper-50 border border-border', iconClass: 'text-primary', key: 'career' },
  { icon: Heart, bgClass: 'bg-secondary/10', iconClass: 'text-secondary', key: 'life' },
] as const
...
export default async function WorkshopsPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-paper-50 pb-20 pt-32 md:pt-40 border-b border-border">
        ...
        <div className="container-editorial relative z-10">
          ...
          <div className="max-w-4xl space-y-8">
            ...
            {/* Unique Stat Strip Location */}
            <div className="flex flex-wrap gap-10 border-t border-border pt-10 mt-12">
              {[
                { icon: Users, value: '450+', label: t('statsYouth') },
                { icon: CalendarDays, value: '30+', label: t('statsSessions') },
                { icon: Layers, value: '2', label: t('statsFormats') },
      ...
      {/* ── Chapter I: Context (Narrative Focus) ───────────────────────────── */}
      <ProgramContextSection
        ...
        paragraph2={
          <>
            {t('chapter1.paragraph2_start')}
            <strong className="font-semibold text-primary">
              {t('chapter1.paragraph2_strong')}
            </strong>
            {t('chapter1.paragraph2_end')}
          </>
        }
        // Stats array passed as empty to remove redundancy from this section
        stats={[]} 
      />

      {/* ── Chapter II: Three Pillars ──────────────────────────────────────── */}
                ...
                >
                  <div className="mb-8 flex items-center justify-between">
                    <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${mod.bgClass}`}>
                      <Icon className={`h-6 w-6 ${mod.iconClass}`} strokeWidth={1.5} />
                    </div>
      ...
      <section className="section bg-paper-50 py-32">
        <div className="container-editorial">
          <div className="mx-auto max-w-4xl text-center">
            <MessageCircle className="mx-auto mb-10 h-12 w-12 text-secondary" strokeWidth={1.5} />
          ...
          <div className="bg-paper-50 border border-border rounded-[3rem] p-12 md:p-20">
            <div className="grid items-center gap-12 lg:grid-cols-2">
              ...
              <div className="flex flex-col gap-4 lg:items-end">
                <Link href="/events" className="btn btn-primary h-16 px-10 w-full lg:w-auto text-lg group">
                  {t('cta.primary')}
                  <ArrowRight className="ml-3 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link href="/join" className="btn btn-outline h-16 px-10 w-full lg:w-auto text-lg">
                  {t('cta.secondary')}
                </Link>
              </div>
            </div>
